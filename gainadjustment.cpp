...
void GainAdjustment::runProcessor(float* buffer, int bufferSize){

	for (int i = 0; i < bufferSize; i++){
		buffer[i] = (buffer[i])*scalingFactor; //this alters the sample's distance from the midpoint by the scaling factor
		fixValue(buffer[i]);
	}
}
